...
class Devlogs_Model extends Model
{
    ...
    function showAllDevlogs($min, $max)
    {
        $sql = "SELECT devlog.name, devlog.Tagline, devlog.Type, devlog.devlogImg, devlog.likeCount, devlog.commentCount,
                devlog.devLogID, freegame.gameName FROM devlog INNER JOIN freegame ON freegame.gameID=devlog.gameName WHERE 
                Visibility='public' ORDER BY createdDate DESC
                LIMIT $min, $max";
    ...
    function showAllSortedDevlogs($sort, $Sorder)
    {
        $sql = "SELECT devlog.name, devlog.Tagline, devlog.Type, devlog.devlogImg, devlog.likeCount, devlog.commentCount,
